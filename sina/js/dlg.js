...
dlg.showLoading = function(top,left)
{
	if(!dlg.spin){
		dlg.spin = new SpinCom();
	}
	dlg.spin.show(top,left);
}
...
dlg.open = function(url,cb,dlgname,title)
{
	if(typeof(cb)=='function')
	{
		tmp_cb={} ;
		tmp_cb.callback = cb ;
		window.showModalDialog(escape(url),tmp_cb,"width:300;height:300;resizable:yes;unadorned:yes");
	}
	else
	{
		tmp_cb = cb ;
...
dlg.close = function(val)
{
	var cb = null;
	if(opener)
		cb = opener.tmp_cb ; //for firefox
	if(cb==null)
		cb = parent.window.dialogArguments ;//for ie
	if(cb)
	{
		if(cb.callback)
		{
	...
	{
		if(opener && opener.dlg && opener.dlg.call_back)
		{
			opener.dlg.call_back(val) ;
		}
	}
	if(parent)
		parent.window.close() ;
